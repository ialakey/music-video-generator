...
кадрами. Замеры на демо-ролике (1080p/30, CRF 21, одни и те же 6 секунд):

| Настройка | Битрейт | Размер трёхминутного трека |
|-----------|--------:|---------------------------:|
| `grain: 0` (выключено)   |  1.9 Мбит/с |  ~43 МБ |
| `grain_size: 2` (умолчание) | 24 Мбит/с | ~540 МБ |
| `grain_size: 1` (попиксельно) | 63 Мбит/с | ~1.4 ГБ |

Поэтому зерно по умолчанию генерируется блоками по 2 пикселя: выглядит как
плёнка, а не как цифровой шум, и стоит втрое дешевле. Если файл всё равно
великоват:
...
CRF 18 на зернистой картинке раздувает файл втрое против CRF 21 без заметной
разницы на глаз — поэтому умолчание именно 21.
