...
![тот же фрагмент с зерном и без](docs/grain-on-off.png)

*Один и тот же фрагмент, пиксель в пиксель. Еле заметная текстура — и файл
в тринадцать раз больше.*
...
![блоки по 2 пикселя против попиксельного шума](docs/grain-block-size.png)

*Слева шум сгенерирован в половинном разрешении и растянут, справа —
попиксельно. На глаз почти одно и то же, по битрейту — в 2.6 раза.*
